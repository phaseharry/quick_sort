...
void QuickSort::sort() {
	...
	int steps = 0;
	sortHelper(arr, 0, arr.size() - 1, steps);
...
void QuickSort::sortHelper(vector<int>& arr, int low, int high, int &steps) {
	while (low < high) {
		int j = partition(arr, low, high, steps);
		if (j - low < high - j) {
			sortHelper(arr, low, j - 1, steps);
			low = j + 1;
			steps++;
		} else {
			sortHelper(arr, j + 1, high, steps);
			high = j - 1;
			steps++;
		}
	}
...
int QuickSort::partition(vector<int>& arr, int low, int high, int &steps) {
	int pivot = arr[high];
	int i = (low - 1);
